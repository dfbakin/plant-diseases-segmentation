"""SPDNet evaluation using cam_classifier (the only inference path that
exercises the trained spatial cross-attention).

Mirrors eval_spatial_full.py but uses generate_all_cams (classifier
weights projected onto FUSED features) instead of generate_all_seeds
(channel statistics on PRE-fusion features).

For each model:
  1. Generate per-image binary CAM via classifier weights x fused features
     (binary_aggregate="max"; same-class refs from PlantSeg train)
  2. Threshold sweep
  3. CRF parameter sweep on 200-image subset (per-distribution tuned)
  4. Full CRF evaluation
  5. Visualizations (25 images + summary grid)

Why this matters: feat_chmean / feat_chvar use query_merged (FPN features
BEFORE fusion), so they're invariant to the reference and to the choice
of fusion mechanism. cam_classifier uses the FUSED features, so it is
the only fair test of whether spatial cross-attention adds localization
signal beyond what the backbone provides.

Usage:
    python scripts/eval_cam_classifier.py 2>&1 | tee logs/eval_cam_classifier.log
"""
...
CAM_LABEL = f"cam_classifier_{BINARY_AGGREGATE}"
...
def generate_visualizations(rname, seed_dir, threshold, crf_params, viz_dir, label):
    random.seed(42)
    ...
    col_labels = ["Original + GT", f"{CAM_LABEL} CAM",
    ...
    fig.suptitle(
        f"{label} | {CAM_LABEL} | Mean IoU: thr={mean_t:.1%}, CRF={mean_c:.1%}",
...
def main():
    ...
    for run in RUNS:
        ...
        if model.fusion_mode == "spatial":
            print(f"  spatial_attn.gate = {model.spatial_attn.gate.item():.4f}")

        seed_dir = output_base / f"seeds_{CAM_LABEL}_corrected_refs"
        existing = list(seed_dir.glob("*.npy")) if seed_dir.exists() else []

        if len(existing) >= len(val_names) * 0.95:
            print(f"[{CAM_LABEL}] CAMs exist ({len(existing)} files), skipping generation")
        else:
            print(f"[{CAM_LABEL}] Generating CAMs (single-scale, same-class refs, "
                  f"binary_aggregate={BINARY_AGGREGATE})...")
            t0 = time.time()
            generate_all_cams(
            ...
            print(f"[{CAM_LABEL}] CAM generation: {time.time() - t0:.0f}s")

        avail = [n for n in val_names if (seed_dir / f"{n}.npy").exists()]
        print(f"[{CAM_LABEL}] {len(avail)} CAMs available")

        print(f"[{CAM_LABEL}] Threshold sweep...")
        t0 = time.time()
        ...
        print(f"[{CAM_LABEL}] Threshold sweep: {time.time() - t0:.0f}s")
        print(f"[{CAM_LABEL}] Best thr={best_thr:.2f}  "
              f"disease_iou={disease_iou_thr:.2f}%  "
              f"bg_iou={best_at.get('background', 0):.2f}%  "
              f"mIoU={best_at.get('mIoU', 0):.2f}%")

        print(f"[{CAM_LABEL}] CRF parameter sweep ({CRF_SWEEP_IMAGES} images)...")
        t0 = time.time()
        ...
        print(f"[{CAM_LABEL}] CRF sweep: {time.time() - t0:.0f}s")
        print(f"[{CAM_LABEL}] Best CRF: srgb={crf_p['srgb']}, "
              f"bg_thr={crf_p['bg_threshold']}, scale={crf_p['scale_factor']}")
        print(f"[{CAM_LABEL}] CRF sweep disease_iou={best_crf['disease_iou']:.2f}% "
              f"(on {CRF_SWEEP_IMAGES} imgs)")

        with open(output_base / f"crf_sweep_{CAM_LABEL}.json", "w") as f:
            json.dump({"best": best_crf, "top5": crf_results[:5]}, f, indent=2)

        print(f"[{CAM_LABEL}] Full CRF eval (all {len(avail)} images, tuned params)...")
        t0 = time.time()
        crf_ious_disease, crf_ious_bg = [], []
        for name in tqdm(avail, desc=f"CRF {CAM_LABEL}"):
            img = np.array(Image.open(IMAGE_DIR / f"{name}.jpg").convert("RGB"))
            h, w = img.shape[:2]
            gt = np.array(Image.open(GT_DIR / f"{name}.png"))
            if gt.shape[:2] != (h, w):
        ...
        print(f"[{CAM_LABEL}] Full CRF: {time.time() - t0:.0f}s")
        print(f"[{CAM_LABEL}] disease_iou={crf_disease:.2f}%  "
        ...
        viz_dir = Path(f"outputs/visualizations/{rname}_{CAM_LABEL}_crf_corrected_refs")
        generate_visualizations(rname, seed_dir, best_thr, crf_p, viz_dir, label)

        all_metrics[rname] = {
            "label": label,
            "num_images": len(avail),
            "fusion_mode": model.fusion_mode,
            "spatial_gate": (model.spatial_attn.gate.item()
                              if model.fusion_mode == "spatial" else None),
            "results": run_metrics,
        }

        with open(output_base / f"evaluation_results_{CAM_LABEL}.json", "w") as f:
    ...
    print("\n" + "=" * 110)
    print("SPDNet cam_classifier EVALUATION (corrected same-class refs) — FINAL SUMMARY")
    print("=" * 110)
    print(f"{'Run':<35} {'Fusion':<8} {'Gate':>6} {'Thr':>5} {'DisIoU(thr)':>12} "
    ...
    for rname, m in all_metrics.items():
        ...
        gate_s = f"{m['spatial_gate']:.3f}" if m["spatial_gate"] is not None else "  -  "
        print(f"{m['label']:<35} {m['fusion_mode']:<8} {gate_s:>6} {sm['best_threshold']:>5.2f} "
              ...
              f"{sm['crf_miou']:>9.2f}%")
    print("-" * 110)
    print("Reference: feat_chvar (corrected, channel-variance on PRE-fusion features):")
    print("  Token N=1 PS:    feat_chvar  CRF=37.63%   feat_chmean CRF=38.32%")
    print("  Spatial PS-only: feat_chvar  CRF=36.94%")
    print("  Spatial PS+PV:   feat_chvar  CRF=37.15%")
    print("  MCTformer MC115 (raw CAM):   29.98%")
    print("=" * 110)

    summary_path = Path("outputs/spdnet_plantseg/eval_summary_cam_classifier.json")
